...
admin.post('/blog', async (c) => {
  ...
  createNotification({ type:'info', title:'New blog post', message:`"${post.title}" has been ${body.status==='published'?'published':'saved as draft'}.`, role:'admin', link:'/dashboard/blog' });
...
admin.post('/faq', async (c) => {
  ...
  createNotification({ type:'info', title:'New FAQ added', message:`"${faq.question}"`, role:'admin', link:'/dashboard/faq' });
...
admin.post('/support', async (c) => {
  ...
  createNotification({ type:'warning', title:'New support ticket', message:`"${ticket.subject}" — ${ticket.priority} priority`, role:'admin', link:'/dashboard/support' });
...
admin.patch('/support/:id', async (c) => {
  ...
  if (body.status === 'resolved') {
    createNotification({ type:'success', title:'Ticket resolved', message:`"${supportStore[idx].subject}"`, role:'admin', link:'/dashboard/support' });
  }
